...
def main():
    """
    This script calculates the total duration of each sentence from CSV files,
    and matches it with the corresponding surprisal values
    The output is saved as 'data.csv'.

    Args:
        duration_folder (str): Path to the folder containing duration CSV files.
        surprisal_path (str): Path to the CSV file containing surprisal data.

    Outputs:
        data.csv: A file containing two columns:
                  - "duration": Sentence duration in seconds.
                  - "surprisal": Average surprisal for the sentence.

    Example:
        Input:
            - duration_folder: Contains 'sentence1.csv', 'sentence2.csv', etc.
            - surprisal_path: A CSV with columns "Sentence" and "Surprisal".
        Output:
            data.csv:
            duration,surprisal
            4.52,6.12
    ...
    Usage:
        python get_durations.py duration/ surprisal.csv
    """
    import sys

    duration_folder = sys.argv[1]
    surprisal_path = sys.argv[2]
    output_csv = "data.csv"
